...
- [ ] T001 Confirm a clean baseline before any change: run `yarn check` (expect 0 errors) and
  `yarn test` (expect the full suite green); note the passing test count so regressions are
  detectable later.
- [ ] T002 Smoke-confirm the Bits UI `Dialog` import path resolves in this offline workspace:
...
- [ ] T003 [P] [US1] **(RED)** Write a failing unit test `tests/unit/theme.test.ts` asserting a
...
- [ ] T004 [US1] **(GREEN)** In `src/theme.ts`: add and export the pure
  ...
  **always-explicit** `data-theme` (compute via `resolveTheme(theme, matchMedia('(prefers-color-scheme: dark)').matches)`; never `removeAttribute`); install a `matchMedia('(prefers-color-scheme: dark)')`
  ...
  T003 pass.
- [ ] T005 [US1] Rewrite the stale `src/theme.ts` header comment (`:1-3`) to describe the
...
- [ ] T006 [US1] Add the pre-paint inline `<script>` to `index.html` `<head>` (before the module
...
- [ ] T007 [US1] Ensure the `matchMedia` listener from T004 is installed at boot (called from the
...
- [ ] T008 [US1] In `src/styles/app.css`: delete the entire `@media (prefers-color-scheme: dark)`
...
- [ ] T009 [US1] Verify US1: `tests/unit/theme.test.ts` green; `yarn check` clean;
...
- [ ] T010 [US2] **(RED/guide)** Update `tests/components/suggest.test.ts` to assert the same
  ...
  **jsdom caveat (analyze F1)**: Bits UI's `FocusScope` (Tab-trap) and `ScrollLock` may rely on
  ...
  jsdom assertion.
- [ ] T011 [US2] Rebuild `src/components/SuggestDialog.svelte` on `bits-ui`'s `Dialog`
...
- [ ] T012 [US2] In `src/App.svelte`: remove the suggest-Esc branch, the entire Tab focus-trap
...
- [ ] T013 [P] [US2] Remove `@internationalized/date` from `package.json` `dependencies`
  (FR-002); leave `bits-ui` (now imported) and `svelte`.
- [ ] T014 [P] [US2] Doc-honesty pass (FR-005, H1): confirm `CLAUDE.md` and the constitution
...
- [ ] T015 [US2] Verify US2: `yarn check` clean; `yarn test` — `suggest.test.ts` and the full
...
- [ ] T016 [US3] Replace the blanket `.specify/` line in `.gitignore` with the narrowing pattern
  (research R6): `.specify/*` / `!.specify/memory/` / `.specify/memory/*` /
  `!.specify/memory/constitution.md`.
- [ ] T017 [US3] Stage the file (`git add .specify/memory/constitution.md`) and verify with
...
- [ ] T018 [US4] Refactor `submitForm` in `src/store.svelte.ts:258-286` to **delegate** the note
...
- [ ] T019 [US4] Remove the write-only suggest status (FR-011, data-model): delete
...
- [ ] T020 [US4] Update tests for the removal: grep `tests/components/store.test.ts` (and others)
...
- [ ] T021 [US4] Verify US4: `yarn check` clean; `yarn test` green; confirm no `SuggestStatus`
...
- [ ] T022 [P] Remove the CSS spec-tag comments coupling `src/styles/app.css` to spec history —
  ...
  the build.
- [ ] T023 Final gate: `yarn check` (0 errors) and `yarn test` (full suite green) across all
...
- [ ] T024 [P] Scope-guard (FR-015, H4): confirm the out-of-scope items are untouched — the store
